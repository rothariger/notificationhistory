...
class NotificationRepository @Inject constructor() {
    ...
    fun getAllNotifications(): Flow<List<NotificationEntity>> =
        notificationBox.query().orderDesc(NotificationEntity_.timestamp).build().flow()

    fun getNotificationsByApp(packageName: String): Flow<List<NotificationEntity>> =
        notificationBox.query().equal(
                NotificationEntity_.appPackageName,
                packageName,
                QueryBuilder.StringOrder.CASE_SENSITIVE
            ).orderDesc(NotificationEntity_.timestamp).build().flow()
    ...
    fun getUnreadNotificationCountByApp(packageName: String): Flow<Int> =
        notificationBox.query().equal(
                ...
                packageName,
                QueryBuilder.StringOrder.CASE_SENSITIVE
            ).equal(NotificationEntity_.isRead, false).build().flow().map { it.size }
    ...
    suspend fun insertNotification(notification: NotificationEntity) = withContext(Dispatchers.IO) {
        ...
        val existingNotification = notificationBox.query().equal(
                NotificationEntity_.uniqueKey,
                notificationToInsert.uniqueKey,
                QueryBuilder.StringOrder.CASE_SENSITIVE
            ).build().findFirst()
    ...
    suspend fun isDuplicate(packageName: String, title: String?, content: String?): Boolean =
        withContext(Dispatchers.IO) {
            // Buscar notificaciones con el mismo paquete, título y contenido
            val query = notificationBox.query().equal(
                    NotificationEntity_.appPackageName,
                    packageName,
                    QueryBuilder.StringOrder.CASE_SENSITIVE
                )
            ...
            if (!title.isNullOrEmpty()) {
                query.equal(
                    NotificationEntity_.title, title, QueryBuilder.StringOrder.CASE_SENSITIVE
                )
            } else {
            ...
            if (!content.isNullOrEmpty()) {
                query.equal(
                    NotificationEntity_.content, content, QueryBuilder.StringOrder.CASE_SENSITIVE
                )
            } else {
    ...
    suspend fun markAllAsRead(packageName: String) = withContext(Dispatchers.IO) {
        val notifications = notificationBox.query().equal(
                NotificationEntity_.appPackageName,
                packageName,
                QueryBuilder.StringOrder.CASE_SENSITIVE
            ).equal(NotificationEntity_.isRead, false).build().find()
    ...
    suspend fun deleteNotificationsByApp(packageName: String) = withContext(Dispatchers.IO) {
        val notificationsToDelete = notificationBox.query().equal(
                NotificationEntity_.appPackageName,
                packageName,
                QueryBuilder.StringOrder.CASE_SENSITIVE
            ).build().find()
    ...
    suspend fun notificationExists(uniqueKey: String): Boolean = withContext(Dispatchers.IO) {
        val count = notificationBox.query().equal(
                NotificationEntity_.uniqueKey, uniqueKey, QueryBuilder.StringOrder.CASE_SENSITIVE
            ).build().count()
